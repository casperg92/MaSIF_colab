...
def save_protein_batch_single(protein_pair_id, P, save_path, pdb_idx):
    ...
    predictions = torch.sigmoid(P["iface_preds"]) if "iface_preds" in P.keys(
    ...
    labels = P["labels"].view(
        -1, 1) if P["labels"] is not None else 0.0 * predictions

    coloring = torch.cat([inputs, embedding, predictions, labels], axis=1)
    save_vtk(str(save_path / pdb_id) + f"_pred_emb{emb_id}",
             xyz,
             values=coloring)
...
def generate_matchinglabels(args, P1, P2):
    if args.random_rotation:
        P1["xyz"] = torch.matmul(P1["rand_rot"].T,
                                 P1["xyz"].T).T + P1["atom_center"]
        P2["xyz"] = torch.matmul(P2["rand_rot"].T,
                                 P2["xyz"].T).T + P2["atom_center"]
...
def compute_loss(args, P1, P2, n_points_sample=16):
    ...
    if args.search:
        ...
        pos_xyz_dists = (((pos_xyz1[:, None, :] -
                           pos_xyz2[None, :, :])**2).sum(-1).sqrt())
...
def iterate(
    ...
    for it, protein_pair in enumerate(
            dataset):  # , desc="Test " if test else "Train")):
        protein_batch_size = protein_pair.atom_coords_p1_batch[-1].item() + 1
        if save_path is not None:
            batch_ids = pdb_ids[total_processed_pairs:total_processed_pairs +
                                protein_batch_size]
        ...
        for protein_it in range(protein_batch_size):
            ...
            P2 = None if args.single_protein else extract_single(
                P2_batch, protein_it)
            ...
            if args.random_rotation:
                ...
                P1["xyz"] = (torch.matmul(P1["rand_rot"],
                                          P1["xyz"].T).T).contiguous()
                P1["normals"] = (torch.matmul(P1["rand_rot"],
                                              P1["normals"].T).T).contiguous()
                if not args.single_protein:
                    P2["rand_rot"] = protein_pair.rand_rot2.view(-1, 3, 3)[0]
                    P2["atom_center"] = protein_pair.atom_center2.view(
                        -1, 1, 3)[0]
                    P2["xyz"] = P2["xyz"] - P2["atom_center"]
                    P2["xyz"] = (torch.matmul(P2["rand_rot"],
                                              P2["xyz"].T).T).contiguous()
                    P2["normals"] = (torch.matmul(
                        P2["rand_rot"], P2["normals"].T).T).contiguous()
            else:
                P1["rand_rot"] = torch.eye(3, device=P1["xyz"].device)
                P1["atom_center"] = torch.zeros((1, 3),
                                                device=P1["xyz"].device)
                if not args.single_protein:
                    P2["rand_rot"] = torch.eye(3, device=P2["xyz"].device)
                    P2["atom_center"] = torch.zeros((1, 3),
                                                    device=P2["xyz"].device)
            ...
            try:
                a = P1["labels"]
                if a is not None:
                    loss, sampled_preds, sampled_labels = compute_loss(
                        args, P1, P2)
                else:
            ...
            if it == protein_it == 0 and not test:
                ...
                for d, features in enumerate(P1["input_features"].T):
                    summary_writer.add_histogram(f"Input features/{d}",
                                                 features)

            if save_path is not None:
                save_protein_embedding_single(batch_ids[protein_it],
                                              P1,
                                              save_path,
                                              pdb_idx=1)
                if not args.single_protein:
                    save_protein_batch_single(batch_ids[protein_it],
                                              P2,
                                              save_path,
                                              pdb_idx=2)
...
def iterate_surface_precompute(dataset, net, args):
    ...
    for it, protein_pair in enumerate(dataset):
        ...
        if args.random_rotation:
            ...
            P1["xyz"] = (torch.matmul(P1["rand_rot"].T, P1["xyz"].T).T +
                         P1["atom_center"])
            ...
            if not args.single_protein:
                ...
                P2["xyz"] = (torch.matmul(P2["rand_rot"].T, P2["xyz"].T).T +
                             P2["atom_center"])
                P2["normals"] = torch.matmul(P2["rand_rot"].T,
                                             P2["normals"].T).T
