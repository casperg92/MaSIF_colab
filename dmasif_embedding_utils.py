...
def get_model(model_resolution: float = 0.7,
              patch_radius: int = 12) -> Tuple[str, int]:
    """
    ...
    model_config = {
        (9, 1): ('dMaSIF_site_3layer_16dims_9A_100sup_epoch64', 100),
    ...
    model_path, supsampling = model_config.get(
        (patch_radius, model_resolution), (None, None))
...
def generate_descr(model_path: str, output_path: str, pdb_file: str,
                   npy_directory: str, radius: float, resolution: float,
                   supsampling: int):
    """
    ...
    parser.add_argument("--curvature_scales",
                        type=list,
    ...
    # Load test dataset:
    test_dataset = [
        load_protein_pair(args.single_pdb,
                          Path(npy_directory),
                          single_pdb=True)
    ]
